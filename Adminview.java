		...
		save_crop.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try 
				{
					...
					S2 = (PreparedStatement) C.prepareStatement("Insert into crop(crop_id, crop_name, min_rainfall, max_rainfall, min_temp, max_temp, MRP) values(?, ?, ?, ?, ?, ?, ?)");
				...
				//save_to_table("Insert into crop(crop_id,crop_name,min_rainfall, max_rainfall, min_temp, max_temp,MRP) values(id_crop,'name',min_rain,max_rain,min_temp,max_temp,mrp)");
				//String sql = "Insert into crop(crop_id, crop_name, min_rainfall, max_rainfall, min_temp, max_temp, MRP) values(?, ?, ?, ?, ?, ?, ?)";
				//save_to_table(sql);
				}
